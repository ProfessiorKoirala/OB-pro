...
const MainScreen: React.FC<MainScreenProps> = ({ activeUser, initialData, onLogout, onUpdateUser, onUpdateUserEmail, onDeleteUser, onPremiumFeatureClick }) => {
    ...
    if (isDesktop) { return <DesktopLayout appData={appData} setAppData={setAppData} activeUser={activeUser} onUpdateUser={onUpdateUser} onLogout={onLogout} onUpdateUserEmail={onUpdateUserEmail} syncStatus={syncStatus} profileData={businessProfile} setProfileData={(p)=>updateState('businessProfile', p)} onUpdateBusinessProfile={(p)=>updateState('businessProfile', p)} pendingOrders={pendingOrders as any} setPendingOrders={setPendingOrders} isVatEnabled={settings.isVatEnabled} setIsVatEnabled={(v)=>updateSettings({isVatEnabled: typeof v === 'function' ? v(settings.isVatEnabled) : v})} handleClearAllData={()=>{}} handleDeleteTable={()=>{}} /> }
