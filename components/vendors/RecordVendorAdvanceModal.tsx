...
interface RecordVendorAdvanceModalProps {
    onClose: () => void;
    onSave: (data: AdvanceData) => void;
}

const RecordVendorAdvanceModal: React.FC<RecordVendorAdvanceModalProps> = ({ onClose, onSave }) => {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b">
                    <h2 className="text-xl font-bold text-text-primary">Give Advance to Vendor</h2>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label htmlFor="amount" className="block text-sm font-medium text-gray-700 mb-1">Advance Amount (₹)</label>
                        <input
                            type="number"
                            name="amount"
                            id="amount"
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        <button type="submit" className="px-6 py-2 bg-secondary text-white font-bold rounded-lg hover:bg-green-700">Save Advance</button>
                    </div>
                </form>
            </div>
        </div>
...
export default RecordVendorAdvanceModal;
